...
class TestIcosahedron(TestCase):
    ...
    def test_get_2fold_axis(self):
        i = Icosahedron(central_atom="Cu", outer_atoms="Cu", shell_distance=2.54, num_shells=3)
        print(len(i))
        i.plot(rotate=False)
        plt.show()
...
class TestFCC(TestCase):
    ...
    def test_6fold_axes(self):
        f = FCC(atom1="Cu", atom2="Cu", lattice_parameter=3.59, radius=11)
        print(len(f))
...
class TestBCC(TestCase):
    ...
    def test_6fold_axes(self):
        b = BCC(atom1="Fe", atom2="Fe", lattice_parameter=2.856, radius=11)
        print(len(b))
        #b.plot(rotate=False)
        #plt.show()
...
class TestCluster(TestCase):
    def test_add_disorder(self):
        b = BCC(atom1="Cu", atom2="Cu", lattice_parameter=3.59, radius=3)
        print(len(b))
